...
interface WaveUniforms {
  ...
  waveColorLow: THREE.Uniform<THREE.Color>
...
interface DitheredWavesProps {
  ...
  waveColorLow: [number, number, number]
  colorNum: number
  pixelSize: number
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<WaveUniforms>({
    ...
    waveColorLow: new THREE.Uniform(new THREE.Color(...waveColorLow)),
  ...
  const prevColorLow = useRef([...waveColorLow])
  useFrame(({ clock }) => {
    ...
    if (!prevColorLow.current.every((v, i) => v === waveColorLow[i])) {
      u.waveColorLow.value.set(...waveColorLow)
      prevColorLow.current = [...waveColorLow]
    }
...
export default function Dither({
  ...
  waveFrequency = 3,
  waveAmplitude = 0.3,
  waveColor = [1.0, 0.36, 0.12],
  waveColorLow = [0.04, 0.04, 0.04],
  colorNum = 6,
  pixelSize = 2,
